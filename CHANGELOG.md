...
- 2026-09-23: `conformance/COVERAGE.md` maps every normative clause of the protocol
  ...
  trees, not their text), or the summary disagrees with the rows, so a protocol edit that
  adds or removes a clause fails CI until its rows follow. The ungraded rows are the
  backlog for new checks. The audit also found checks weaker than their titles, filed as
  issue #134.
...
- 2026-09-22: vehicles 2 (issue #77, plugin 0.8.0, no protocol change). Each
  ...
  replacement (the new wheel's own parts walked like any part's), its lists of swaps and
  problems sharing a byte budget with complete counts beside them. `core.vehicle.destroy` gains `state`, and `damageType` and `ammo` from the
