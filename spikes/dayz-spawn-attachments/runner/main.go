...
// spawn probe (../harness/VyshkaSpawnProbe.c) with the Vyshka mod
// loaded, follows the script log for the probe's lines, and prints them.
//
//	go run ./spikes/dayz-spawn-attachments/runner [-server DIR] [-mod DIR] [-profiles DIR]
//
// The probe's lines go to stdout, tab separated, one per slot, per phase,
// per firearm, per case, and a finished line; everything else goes to stderr. The exit
...
const (
	tag        = "VYSHKA_SPAWN"
	probeEntry = "\tVyshkaSpawnProbe.Run();"
	prefix     = "vyshkaSpawnSpike"
	cfgName    = "vyshka_spawn_spike_serverDZ.cfg"
)
...
func run() error {
	...
	profiles := flag.String("profiles", "spikes/dayz-spawn-attachments/results/profile", "profile directory the server writes to")
	probePath := flag.String("probe", "spikes/dayz-spawn-attachments/harness/VyshkaSpawnProbe.c", "the probe appended to the mission's init.c")
	...
	// No -freezecheck: the index build and the equips hold the main thread on
...
func serverConfig(mission string) string {
	return strings.Join([]string{
		`hostname = "vyshka-spawn-spike";`,
		`password = "";`,
		`passwordAdmin = "";`,
		`description = "Vyshka spawn attachments spike";`,
		`enableWhitelist = 0;`,
		`maxPlayers = 1;`,
		`verifySignatures = 0;`,
		`forceSameBuild = 0;`,
