...
// world probe (../harness/VyshkaWorldProbe.c), follows the script log for
// the probe's lines, and prints them. No mod is loaded: the probe uses the
// engine's own script API only.
//
//	go run ./spikes/dayz-world-clock/runner [-server DIR] [-profiles DIR]
//
// The probe's lines go to stdout, tab separated, three per sample, one per
// scheduled change, and a finished line; everything else goes to stderr. The exit
...
const (
	tag        = "VYSHKA_WORLD"
	probeEntry = "\tVyshkaWorldProbe.Run();"
	prefix     = "vyshkaWorldSpike"
	cfgName    = "vyshka_world_spike_serverDZ.cfg"
)
...
func run() error {
	...
	profiles := flag.String("profiles", "spikes/dayz-world-clock/results/profile", "profile directory the server writes to")
	probePath := flag.String("probe", "spikes/dayz-world-clock/harness/VyshkaWorldProbe.c", "the probe appended to the mission's init.c")
	port := flag.Int("port", 2402, "game port for the server")
	timeout := flag.Duration("timeout", 30*time.Minute, "how long the boot and the probe may take together")
	flag.Parse()
...
func serverConfig(mission string) string {
	return strings.Join([]string{
		`hostname = "vyshka-world-spike";`,
		`password = "";`,
		`passwordAdmin = "";`,
		`description = "Vyshka world clock spike";`,
		`enableWhitelist = 0;`,
		`maxPlayers = 1;`,
		`verifySignatures = 0;`,
		`forceSameBuild = 0;`,
