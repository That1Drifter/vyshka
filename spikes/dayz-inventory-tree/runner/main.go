...
// inventory tree probe (../harness/VyshkaInventoryProbe.c) with the Vyshka mod
// loaded, follows the script log for the probe's lines, and prints them.
//
//	go run ./spikes/dayz-inventory-tree/runner [-server DIR] [-mod DIR] [-profiles DIR]
//
// The probe's lines go to stdout, tab separated, one per slot, per phase,
// per depth, and a finished line; everything else goes to stderr. The exit
...
const (
	tag        = "VYSHKA_INVENTORY"
	probeEntry = "\tVyshkaInventoryProbe.Run();"
	prefix     = "vyshkaInventorySpike"
	cfgName    = "vyshka_inventory_spike_serverDZ.cfg"
)
...
func run() error {
	...
	profiles := flag.String("profiles", "spikes/dayz-inventory-tree/results/profile", "profile directory the server writes to")
	probePath := flag.String("probe", "spikes/dayz-inventory-tree/harness/VyshkaInventoryProbe.c", "the probe appended to the mission's init.c")
	loadoutPath := flag.String("loadout", "spikes/dayz-inventory-tree/harness/VyshkaLoadout.c", "the loadout helper appended after the probe")
	...
	loadout, err := os.ReadFile(resolve(*loadoutPath))
	if err != nil {
		return fmt.Errorf("reading the loadout helper: %w", err)
	}
	probe = append(probe, byte(10), byte(10))
	probe = append(probe, loadout...)
	...
	// No -freezecheck: the gearing and the builds hold the main thread on
	// purpose, and the stall is part of what is measured.
	start := time.Now()
...
func serverConfig(mission string) string {
	return strings.Join([]string{
		`hostname = "vyshka-inventory-spike";`,
		`password = "";`,
		`passwordAdmin = "";`,
		`description = "Vyshka inventory tree spike";`,
		`enableWhitelist = 0;`,
		`maxPlayers = 1;`,
		`verifySignatures = 0;`,
		`forceSameBuild = 0;`,
