...
| `vyshka.vehicle.repair` | vehicle | none | `parts` (default true) | `vehicle`, `type`, `kind`, `position`, `before` and `after` (each `{ state, health }`), `parts` (how many were repaired or swapped), `replaced` (one `{ slot, from, to }` per ruined wheel swapped back to its intact class), `problems` (one `{ class, slot, reason }` per part that could not be); the engine's own full-health call on every damage zone, which lifts a destruction, then with `parts` every attached part and the parts on those. Fluids are left to the refuel action, and a missing part stays missing |
...
snapshot may carry (around a thousand vehicles: 52 took 11 464 bytes on a live run) is sent without `displayName` and
`fluids` instead, so every vehicle stays in it, and the plugin logs that once per run.
