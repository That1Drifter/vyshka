...
func TestEnvelopeOperationsRefuseASupersededSession(t *testing.T) {
	...
	t.Run("AdvanceInbound", func(t *testing.T) {
		_, err := st.AdvanceInbound(ctx, stale.ID, func(ack int64) (int64, int) { return ack + 1, 1 })
...
func TestAdvanceInboundClassifiesAgainstCommittedState(t *testing.T) {
	ctx := context.Background()
	st := migrated(t)
	serverID := enrolledServer(t, st, "committed state")
	session := startSession(t, st, serverID, "token-hash")

	ack, err := st.AdvanceInbound(ctx, session.ID, func(ack int64) (int64, int) {
		if ack != 0 {
	...
	if err != nil || ack != 2 {
		t.Fatalf("first advance: ack = %d, err = %v, want 2", ack, err)
	}
	...
	if err != nil || ack != 2 {
		t.Fatalf("second advance: ack = %d, err = %v, want 2", ack, err)
	}

	// A classifier that tries to go backwards is refused rather than obeyed.
	ack, err = st.AdvanceInbound(ctx, session.ID, func(int64) (int64, int) { return 1, 1 })
	if err != nil {
		t.Fatalf("third advance: %v", err)
	}
	if ack != 2 {
		t.Errorf("ack = %d after a classifier returned 1, want it held at 2", ack)
	}
}
...
func TestSequenceStateResetsWithTheSession(t *testing.T) {
	...
	if _, err := st.AdvanceInbound(ctx, first.ID, func(int64) (int64, int) { return 7, 7 }); err != nil {
		t.Fatalf("advance inbound: %v", err)
	}
	...
	inbound, err := st.AdvanceInbound(ctx, second.ID, func(ack int64) (int64, int) { return ack, 0 })
	if err != nil {
		t.Fatalf("read inbound ack: %v", err)
	}
	if inbound != 0 {
		t.Errorf("inbound ack = %d in a fresh session, want 0", inbound)
	}
}
