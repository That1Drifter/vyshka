...
CREATE TABLE IF NOT EXISTS event_identity_backfill (
    ...
    after_id   TEXT NOT NULL,            -- the last event id indexed; '' before the first
    through_id TEXT NOT NULL             -- the newest event id at migration time
);
...
CREATE TABLE IF NOT EXISTS audit_notifications (
    audit_id TEXT PRIMARY KEY REFERENCES audit_records (id) ON DELETE CASCADE
);
