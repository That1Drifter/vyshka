-- State snapshots (spec/protocol.md section 8.3), Postgres variant of
-- 0010_state.sql: identical except for the spelling of the seq column.
...
-- An identity column draws from a sequence that never hands a value out
-- twice, so "newer" stays monotone under the trimming below. This is the
-- Postgres spelling of the SQLite file's AUTOINCREMENT; the two files land
-- the same schema.
--
